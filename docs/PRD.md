...
**Judge Skills** (SKILL.md folders under `src/debate/skills/judge/` loaded by `SkillLoader`; verdict logic in `verdict.py`):
| Skill | Type | Description |
|-------|------|-------------|
| `enforce_debate_mechanics` | deterministic | Rule-based check: citation presence, no sycophantic phrases, fallacy acknowledgement (round 2+). Emits `ReprimandMessage` on rule failure; no LLM call. |
| `evaluate_persuasion_score` | llm_prompt | Scores logic/citation/rhetoric (0–1) via evaluate LLM with ZERO-ANCHORING. Up to 3 context blocks pre-assembled by the agent (FEEDBACK ENFORCEMENT, NOVELTY CHECK, REFUTATION CHECK) are passed in as `{{ var }}` placeholders. |
| `generate_judge_feedback` | llm_prompt | Calls route LLM for 2–3 sentences of feedback (weakest dim + repetition call-out + mandatory next-round instruction). Output becomes `RoutingMessage.judge_feedback`. |
| `compose_next_turn_prompt` | deterministic | Builds the literal `prompt_for_next` handoff string (no LLM call) — `"It is your turn now, {next_agent}. Respond directly to the previous argument."` plus an optional REMINDER block when the agent has prior feedback. Split out from the original `RouteTurn` in commit `651a5a8`. |
| `DeclareVerdict` (Python, in `verdict.py`) | — | Deterministic cumulative scoring; citation-strength tie-break; four-section justification (KEY CLASHES / FEEDBACK ADHERENCE / SCORING BREAKDOWN / FINAL CONCLUSION) generated by `verdict_llm`. |
